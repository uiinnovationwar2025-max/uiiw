...
const Hero = () => {
  return (
    <main className="relative flex flex-col min-h-screen items-center justify-center overflow-hidden">
      {/* Background Objects */}
      {/* Desktop */}
      <div className="absolute -top-24 hidden lg:block -left-14 w-5xl h-[72rem]">
        <Image src={"/landing-hero/object-1.svg"} alt="object" fill />
      </div>
      <div className="absolute -top-24 right-0 hidden lg:block w-2xl h-[72rem]">
        <Image src={"/landing-hero/object-2.svg"} alt="object" fill />
      </div>

      {/* Mobile */}
      <div className="absolute -top-48 block lg:hidden w-full h-[36rem]">
        <Image src={"/landing-hero/object-3.svg"} alt="object" fill />
      </div>
      <div className="absolute -bottom-64 -z-40 block lg:hidden w-full h-[36rem]">
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-screen-xl px-6 lg:px-12 space-y-10 lg:space-y-0 lg:space-x-12 z-10">
        ...
        <div className="w-full lg:w-3/5 flex flex-col justify-center space-y-6 text-center lg:text-left">
          ...
          <p className="font-primary font-medium text-sm sm:text-base lg:text-lg text-secondary-2 text-s7">
            UI Innovation War adalah wadah penyaluran intelektualitas dan pemikiran
            kritis dari pelajar dan mahasiswa melalui media kompetisi studi kasus
            bisnis dan perancangan bisnis untuk menghasilkan strategi bisnis yang
            adaptif
          </p>
          <Button
            variant="primary"
